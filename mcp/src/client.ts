...
export const HOPFAST_API_URL =
  process.env.HOPFAST_API_URL ?? 'http://localhost:8080';
...
export async function hopfastFetch<T>(
  path: string,
  options?: RequestInit,
): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new HopFastApiError(
      response.status,
      `HopFast API error (${response.status} ${path}): ${text}`,
    );
  }
