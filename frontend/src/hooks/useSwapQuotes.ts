...
export function useSwapQuotes(activeWalletAddress: string | null) {
  const [quotes, setQuotes] = useState<
    Partial<Record<ProviderKey, QuoteResult | null>>
  >({});
  const [quotingProviders, setQuotingProviders] = useState<Set<ProviderKey>>(
    new Set()
  );
  const [retryingProviders, setRetryingProviders] = useState<Set<ProviderKey>>(
    new Set()
  );
  ...
  const retryTimeoutRefs = useRef<
    Partial<Record<ProviderKey, ReturnType<typeof setTimeout>>>
  >({});
  ...
  const bestQuote = (() => {
    if (selectedProvider && quotes[selectedProvider])
      return quotes[selectedProvider]!;
    const available = LIVE_PROVIDERS.map((p) => quotes[p]).filter(
  ...
  const fetchQuote = useCallback(
    async (currentDraft: SwapDraft) => {
      ...
      Object.values(retryTimeoutRefs.current).forEach(
        (t) => t && clearTimeout(t)
      );
      ...
      const eligibleProviders = LIVE_PROVIDERS;
      setQuotingProviders(new Set(eligibleProviders));
      setRetryingProviders(new Set());
      setQuoteCountdown(null);

      const walletAddr = activeWalletAddress;
      pendingRef.current = eligibleProviders.length;

      eligibleProviders.forEach(async (provider) => {
        ...
        try {
          ...
          pendingRef.current--;
          if (pendingRef.current === 0 && autoRefreshRef.current)
  ...
  const triggerFetchImmediate = useCallback(
    (next: SwapDraft) => {
      ++roundRef.current;
      Object.values(retryTimeoutRefs.current).forEach(
        (t) => t && clearTimeout(t)
  ...
  const setupAmountDebounce = useCallback(
    (draft: SwapDraft) => {
  ...
  useEffect(() => {
    ...
    if (
      activeWalletAddress &&
      draftRef.current &&
      isValidSwapInput(draftRef.current)
    ) {
  ...
  const setAutoRefresh = useCallback((enabled: boolean) => {
    ...
    setQuoteCountdown((current) => (enabled ? current : null));
  }, []);
